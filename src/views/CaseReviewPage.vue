...
async function handleSubmitReview(item: CaseReviewItem, result: 'approved' | 'rejected') {
  ...
  try {
    ...
    reviewComment.value = ''
    // 刷新详情数据
    const updated = await getReview(selectedProjectId.value, currentReview.value.id)
    ...
    <div class="content-area">
            ...
            <template #default="{ row }">
              <span class="mode-badge" :class="row.review_mode">
                <span class="material-symbols-outlined" style="font-size: 14px">
                  {{ row.review_mode === 'parallel' ? 'group' : 'person' }}
                </span>
          ...
          <el-table-column label="评审进度" width="210">
            <template #default="{ row }">
              <div class="progress-cell">
                <el-progress
                  :percentage="
                    row.case_total_count > 0
                      ? Math.round(
                          ((row.approved_count + row.rejected_count + row.needs_update_count) /
                            row.case_total_count) *
                            100,
                        )
                      : 0
                  "
          ...
          <el-table-column label="通过率" width="80" align="center">
            <template #default="{ row }">
              <span
                class="rate-chip"
                :class="{
                  high: row.pass_rate >= 80,
                  low: row.pass_rate < 50 && row.case_total_count > 0,
                }"
              >
      ...
      <div v-else class="empty-state">
        ...
        <el-button type="primary" size="large" class="empty-create-btn" @click="openCreateDialog">
          <span class="material-symbols-outlined" style="font-size: 18px; margin-right: 6px">
            add_circle
          </span>
          创建第一个评审计划
        </el-button>
...
:deep(.progress-cell .el-progress-bar__outer) {

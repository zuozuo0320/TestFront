...
import type { Module, ModuleTreeResp } from './types'

// Get module tree with case counts
export async function getModuleTree(projectId: number) {
  const { data } = await apiClient.get<ModuleTreeResp>(`/projects/${projectId}/modules`)
  return data
}

// Create a module
export async function createModule(projectId: number, parentId: number, name: string) {
  const { data } = await apiClient.post<{ data: Module }>(`/projects/${projectId}/modules`, {
    parent_id: parentId,
    name,
  })
  return data
...
export async function moveModule(
  ...
  const { data } = await apiClient.put(`/projects/${projectId}/modules/${moduleId}/move`, {
    parent_id: parentId,
...
export async function deleteModule(projectId: number, moduleId: number) {
  const { data } = await apiClient.delete(`/projects/${projectId}/modules/${moduleId}`)
  return data
}

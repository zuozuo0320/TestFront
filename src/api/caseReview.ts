...
export async function listReviews(projectId: number, params: ReviewListParams) {
    const { data } = await apiClient.get<{ items: CaseReview[]; total: number; page: number; pageSize: number }>(
        `/projects/${projectId}/case-reviews`,
        { params },
    )
    return data
}
...
export async function getReview(projectId: number, reviewId: number) {
    const { data } = await apiClient.get<CaseReview>(`/projects/${projectId}/case-reviews/${reviewId}`)
    return data
}

/** 更新评审计划 */
export async function updateReview(projectId: number, reviewId: number, payload: UpdateReviewPayload) {
...
/** 复制评审计划 */
export async function copyReview(projectId: number, reviewId: number, payload: { name?: string; include_cases?: boolean; reset_reviewers?: boolean }) {
    const { data } = await apiClient.post<CaseReview>(`/projects/${projectId}/case-reviews/${reviewId}/copy`, payload)
    return data
}

/** 评审项列表 */
export async function listReviewItems(projectId: number, reviewId: number, params: ReviewItemListParams) {
    const { data } = await apiClient.get<{ items: CaseReviewItem[]; total: number; page: number; pageSize: number }>(
...
/** 关联用例 */
export async function linkItems(projectId: number, reviewId: number, items: LinkItemEntry[], autoSubmit = false) {
    const { data } = await apiClient.post(`/projects/${projectId}/case-reviews/${reviewId}/items/link`, { items, auto_submit: autoSubmit })
    return data
}

/** 移除用例 */
export async function unlinkItems(projectId: number, reviewId: number, itemIds: number[]) {
    const { data } = await apiClient.post(`/projects/${projectId}/case-reviews/${reviewId}/items/unlink`, { item_ids: itemIds })
    return data
}

/** 单条评审提交 */
export async function submitItemReview(projectId: number, reviewId: number, itemId: number, result: string, comment: string) {
...
/** 批量评审 */
export async function batchReview(projectId: number, reviewId: number, itemIds: number[], result: string, comment: string) {
...
/** 批量改评审人 */
export async function batchReassign(projectId: number, reviewId: number, itemIds: number[], reviewerIds: number[]) {
...
/** 评审记录 */
export async function listItemRecords(projectId: number, reviewId: number, itemId: number, params: { page: number; pageSize: number; round_no?: number }) {
    const { data } = await apiClient.get<{ items: CaseReviewRecord[]; total: number; page: number; pageSize: number }>(
        `/projects/${projectId}/case-reviews/${reviewId}/items/${itemId}/records`,
        { params },
    )
    return data
}
